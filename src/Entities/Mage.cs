...
namespace rpg_poo.src.Entities
{
    ...
    {
        public Mage(string heroType, int life, string name) : base(heroType, life, name)
        {
        ...
        public override string Attack(Hero opponent, int hitDamage)
        {
            ...
            {
                string status = $"{Name} cast magic on, {opponent.Name}. Dealing {critical} critical damage.";
                opponent.Life -= critical;
                return status;
            }
            else
            {
                string status = $"{Name} cast magic on, {opponent.Name}. Causing {hitDamage} damage.";
